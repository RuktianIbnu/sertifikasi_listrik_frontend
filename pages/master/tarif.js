...
const AddTarif = loadable(() =>
  import("../../components/tarif/AddTarif")
);
const EditTarif = loadable(() =>
  import("../../components/tarif/EditTarif")
);
...
function Tarif() {
  ...
  const deleteTarif = async (idTarif) => {
    try {
      ...
      if (status === 200) {
        addToast("Berhasil hapus tarif", { appearance: "success" });
        fetchTarif();
      }
    } catch (error) {
      addToast(errorHandler(error), { appearance: "error" });
        ...
        <div>
          <div className="bg-white shadow w-full rounded px-6 py-10">
            ...
            <div className="flex flex-row break-normal items-center border-b-2 px-3 py-2">
              ...
              <p className="text-sm mr-2 mb-0 font-bold w-11/12">Tarif per KWH</p>
            ...
            {tarif.map((item, index) => (
              ...
              >
                ...
                <p className="text-sm mr-2 mb-0 w-11/12">{item.daya}</p>
                <p className="text-sm mr-2 mb-0 w-11/12">{item.tarifperkwh}</p>
